              ...
              <div>
                {filteredMeetings.filter(m => m.status === "Completed").length === 0 ? (
                  ...
                  {filteredMeetings.filter(m => m.status === "Completed").map((m) => (
                    <div key={m.id} className="flex flex-col md:flex-row md:items-center justify-between py-6 md:py-8 border-b border-[var(--border-color)] last:border-0 gap-4 md:gap-6 opacity-85 max-md:-mx-4 max-md:px-4 -mx-6 px-6">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
